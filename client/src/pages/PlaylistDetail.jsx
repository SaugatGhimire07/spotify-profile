  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter out tracks without IDs and limit to 100 tracks (API limit)
        const trackIds = playlistResponse.data.tracks.items
        ...
        const features = featuresResponse.data.audio_features.reduce(
          (acc, cur) => {
            if (!cur) return acc;
            return {
              danceability: acc.danceability + cur.danceability,
              energy: acc.energy + cur.energy,
              speechiness: acc.speechiness + cur.speechiness,
              acousticness: acc.acousticness + cur.acousticness,
              instrumentalness: acc.instrumentalness + cur.instrumentalness,
              liveness: acc.liveness + cur.liveness,
              valence: acc.valence + cur.valence,
              count: acc.count + 1,
            };
          },
          {
        ...
        // Calculate averages
        const averageFeatures = Object.keys(features).reduce((acc, key) => {
          if (key !== "count") {
            acc[key] = features[key] / features.count;
          }
          return acc;
        }, {});
                  ...
                  <div className="w-[300px] flex flex-col items-center">
                    ...
                    {audioFeatures && (
                      <div className="mt-[60px] w-full">
                        <h3 className="text-white text-[18px] font-bold mb-[20px] text-center">
                          Audio Features
                        </h3>
